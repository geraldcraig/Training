...
def predict():
    ...
    setup_df = pd.DataFrame(
        [pd.Series([input_one, input_two, input_three, input_four, input_five, input_six, input_seven, input_eight])])
    ...
    if output > str(0.5):
        ...
        return render_template('result.html', pred=f'You have a low chance of diabetes which currently considered safe (this is only an example, please consult a certified doctor for any medical advice).\nProbability of having diabetes is {output}')
